...
export function NewsCarousel() {
    ...
    return (
        <Card className="bg-gradient-to-br from-[#1a1f2e] to-[#151923] border-slate-800/50 hover:border-orange-500/30 transition-all duration-300">
            ...
            <CardContent className="space-y-3">
                {currentNews.map((news) => (
                    <div
                        key={news.id}
                        className="p-3 rounded-lg bg-slate-800/30 hover:bg-slate-800/50 transition-all duration-200 cursor-pointer group"
                    >
                        {/* Category & Time */}
                        <div className="flex items-center justify-between mb-2">
                            <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${getCategoryColor(news.category)}`}>
